...
**M1-S6 MySQL/MariaDB shipped policy (scoped; §22.2).** The M1 MySQL backend hydrates only the scoped canonical scalar set — `NULL`, `BOOL`, `I64`, `F64`, `TEXT`, `BYTES` — keyed off the wire **column metadata** (column type + flags + charset + display length), not the driver's ambiguous binary `Value`. `TINYINT(1)` (MySQL's `BOOLEAN` idiom, identified by display length 1) → `BOOL`; a wider signed `TINYINT`/`SMALLINT`/`MEDIUMINT`/`INT`/`BIGINT` → `I64`; `FLOAT`/`DOUBLE` → `F64`; `CHAR`/`VARCHAR`/`TEXT` (non-binary charset) → `TEXT`; `BINARY`/`VARBINARY`/`BLOB` (binary collation 63) → `BYTES`. Every **out-of-scope** column type is a **loud `Unsupported`** naming the column + its MySQL type — NEVER a silent miscast — each pending its policy above landing in a later slice: **`BIGINT UNSIGNED` / any unsigned integer** (the `u64_overflow` policy is deferred — no lossy cast into the signed `I64` domain, a value can exceed `i64::MAX`), **`DECIMAL`/`NEWDECIMAL`** (the `decimal` policy is deferred — never a silent `TEXT` cast), the **`DATE`/`DATETIME`/`TIMESTAMP`/`TIME`/`YEAR`** family (the `naive_datetime_zone` policy is deferred), plus `JSON`/`BIT`/`ENUM`/`SET`/`GEOMETRY`/`VECTOR`. A cell whose driver `Value` variant does not match its column kind is a client-side decode mismatch → `Backend` (NonRetryable), never `ConnectionLost` (so it can never mint a false §19.3 `Indeterminate`). This is the documented policy the S8/S9 DBAL/Eloquent suite inherits (§14–15) — a diagnosable error, not a surprise miscast. Source: `engine/crates/ferro-backend-mysql/src/rowmap.rs`.
...
- **§9/§9.1 the M1 slice plan gains a dedicated CANONICAL TYPE-COVERAGE slice; the tail renumbers S7→S8→S9→S10 (M1 plan amendment, 2026-08-05)** — the M1 execution design originally went S6 (MySQL) → S7 (Doctrine DBAL-4 driver) → S8 (DBAL upstream suite green = the exit gate). S6's close-out research established that the **M0 scalar set (`NULL/BOOL/I64/F64/TEXT/BYTES`) is a hard end-to-end boundary on BOTH backends and in PHP**: PG's `oid_to_tag` raises a loud `Unsupported` for every OID outside it (`ferro-backend-pg/src/rowmap.rs`, e.g. `TIMESTAMPTZ`/`UUID`/`NUMERIC`/`JSONB`), the S6 MySQL policy does the same by column metadata (§9.1 above), and the PHP `M0ValuePolicy` throws `ProtocolException` on every reserved M1 tag (`DECIMAL/DATE/TIME/TIMESTAMP/TIMESTAMPTZ/UUID/JSON/ARRAY/INTERVAL/INET/VECTOR/U64`). Since a Doctrine DBAL suite is saturated with dates, decimals and JSON, the exit gate as written ("the DBAL 4 suite is GREEN modulo a documented allow-list") was **not reachable** — the allow-list would have absorbed most of the suite and the M1 exit condition would have proven little. Type coverage is therefore promoted to its own slice with its own gate, ahead of the driver, because it is a distinct subsystem (§9/§9.1) that BOTH drop-in tiers (§14 DBAL, §15 Eloquent) stand on. **New numbering:** S7 = canonical type coverage (`U64`, `DECIMAL`, `DATE`, `TIME`, `TIMESTAMP`, `TIMESTAMPTZ`, `UUID`, `JSON` end-to-end on both backends + the §9.1 pool-level policies `decimal`/`naive_datetime_zone`/`u64_overflow`/`uuid` + a golden vector per tag in both codecs per charter rule 2; `ARRAY`/`INTERVAL`/`INET`/`VECTOR` stay explicitly deferred as PG-exotic and not required for a green DBAL suite); S8 = the Doctrine DBAL-4 driver, which also carries the S6 deferrals (MySQL `query_stream`, dialect-aware isolation `BEGIN`, errno-on-wire, `HELLO_ACK` pool metadata for backend kind + server version, an imperative `begin/commit/rollBack` client API, `last_insert_id` end-to-end, and the tracker-clean `None`-skip re-evaluation); S9 = the DBAL upstream suite + incompat doc (**still the M1 exit gate**, unchanged in substance); S10 = the conditional accelerator. The §9 canonical table and §9.1 policy set are unchanged — this slice IMPLEMENTS them rather than altering them. See `docs/superpowers/specs/2026-07-28-ferro-m1-execution-design.md` §5.
- **§9.1 the four type policies are CLIENT-SIDE in M1 (no engine knob); `naive_datetime_zone: server` is deferred to S8; `error` is scoped to `TIMESTAMP`; a policy refusal is NOT a wire fault (M1-S7 Task 6)** — §9.1 authors `decimal`/`naive_datetime_zone`/`u64_overflow`/`uuid` as **pool-level** policy. As shipped in M1 they are **client-side**, a `Ferro\Client\Value\TypePolicyOptions` value object threaded `Ferro::connect(types:)` → `Connection(types:)` (defaults unchanged: the safe object forms, `naive_datetime_zone=utc`). *Why:* the S7 wire is lossless **canonical text** (`proto/PROTOCOL.md` §3.2) — the engine renders each backend's native binary form into a policy-INDEPENDENT payload, so nothing in `ferrod` could read a `decimal=string` pool setting. The originally-planned four `PoolConfig` fields + `ferrod` env parsing were therefore **dropped, not deferred**: an operator setting one would observe nothing, while a typo in that inert setting would stop `ferrod` from booting — the worst of both. Pool-level defaults **advertised** to the client via `HELLO_ACK` pool metadata are an **S8 carry** (the same metadata the S6 backend-kind/server-version carry waits on). **`naive_datetime_zone: server` is not implementable in M1** and is rejected loudly by the constructor rather than silently downgraded to `utc`: nothing on the wire carries the backend's session timezone (`HelloAck` is `[engine_version, boot_epoch, features, pools, type_registry_hash]`); it lands with that same S8 metadata. **`naive_datetime_zone: error` has a pinned scope — `TAG_TIMESTAMP` alone** (`TypePolicyOptions::refusesNaiveTimestamp`); `TIMESTAMPTZ`, `DATE` and `TIME` decode normally under it, since an undefined scope would make whole columns unreadable with no escape hatch (the escapes are switching back to `utc`, or reading the column as canonical text verbatim). Finally, the client exception tree splits by **cause**: a **malformed** payload (a `DECIMAL` that is not a number, a truncated `TIMESTAMP`) stays `ProtocolException` (a wire fault), while an operator **policy refusal** is the new `Ferro\Client\Error\TypePolicyException extends FerroException` — deliberately outside the §9.2 Retryable/Indeterminate/NonRetryable branches, because it is raised client-side *after* the statement already succeeded (inside `ExecCodec::decodeRow`, mid-row on the streamed path) and must never be mistaken for a §19.3 fate signal or make S8's DBAL `ExceptionConverter` report a configuration choice as a driver protocol failure. Task 9 folds this into the slice-wide S7 entry.
