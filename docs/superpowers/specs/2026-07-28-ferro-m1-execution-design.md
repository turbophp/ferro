...
- **A PHP Doctrine test harness** (`php/doctrine-dbal/`): the DBAL 4 upstream suite run against a Ferro-backed connection, using the PHP client + a launched `ferrod` (reusing the S8/S9 process harness), against Docker PG + MySQL.
...
Each slice lands **green with practical tests before the next begins**. Dependencies noted. The correctness core (S1–S4) is the critical path; S5 (streaming) and S6 (MySQL) can parallelize after S4; S7 (canonical types) depends on S5+S6 and gates the tiers; S8–S9 (Doctrine) depend on S7; S10 is conditional.
...
- **Build:** hygiene runs at **checkout**, **awaited** (pipelining ahead of the first user statement is DEFERRED — see Gate below, and SPEC §22). A **tainted** conn → full reset (PG `DISCARD ALL`). A **non-tainted, recycled PG** conn → the targeted profile `CLOSE ALL; SET SESSION AUTHORIZATION DEFAULT; RESET ALL; UNLISTEN *; SELECT pg_advisory_unlock_all(); DISCARD TEMP; DISCARD SEQUENCES;` (preserves namespaced prepares; the `CLOSE ALL`/`SET SESSION AUTHORIZATION DEFAULT` additions beyond this doc's originally-authored 5-statement list close a `WITH HOLD`-cursor / role gap — recorded in SPEC §22). A **known-clean** conn → skip hygiene entirely. Drive the decision from the S1/S2 `tainted` bit specifically (**not** "never-pinned" — a cleanly-committed transaction is non-tainted and gets the targeted profile, not a forced full reset; see SPEC §22).
- **Gate:** live PG — a conn that held an advisory lock / a temp table / a `LISTEN` is CLEAN for the next tenant (no leak — the exact v0.1 bugs §7.2 calls out) — **MET** (S3 live tests, including the §7.4 blind-spot backstop: a `set_config(...)`/in-`DO` session mutation invisible to the S2 lexer is still closed by the targeted profile). A non-tainted recycled conn's targeted reset keeps a live `PREPARE` across checkout, where a tainted conn's full reset destroys it — **MET** (S3 proves both directions live). **DEFERRED:** "hygiene is measurably pipelined (issued before the first user statement)" does **not** ship in S3 — it is pushed to a later perf slice, gated on both a recorded bench number and the §7.3 statement cache (unbuilt), whose cached prepares are the thing pipelining would need to preserve without racing the fork's single-slot RFQ atomic. Per charter rule 5 (no speculative optimization), there is nothing yet to gain by pipelining hygiene ahead of the first statement. Hygiene runs **awaited at checkout** for now (as before S3, only with the correct per-state profile). Extend the S4-pool hygiene tests.
...
### S7 — Canonical type coverage (§9/§9.1) beyond the M0 scalar set  *(depends: S5, S6)*  — **inserted 2026-08-05**
**Why this slice exists (plan amendment).** S6's close-out research found the M0 scalar set (`NULL/BOOL/I64/F64/TEXT/BYTES`) is the hard end-to-end boundary on **both** backends *and* in PHP: PG's `oid_to_tag` raises a loud `Unsupported` outside that set (`rowmap.rs`), the S6 MySQL policy does the same (§9.1), and `M0ValuePolicy` throws `ProtocolException` on every reserved M1 tag. A Doctrine suite is saturated with dates, decimals and JSON, so the old S8 exit gate ("DBAL 4 suite green modulo an allow-list") was **not reachable** — the allow-list would have swallowed the suite and the M1 exit condition would have proven little. The type work is a distinct subsystem (§9/§9.1) with its own gate, and BOTH drop-in tiers (DBAL §14, Eloquent §15) stand on it, so it lands as its own slice before the driver.
- **Build:** the DBAL-critical canonical tags end-to-end — `U64`, `DECIMAL`, `DATE`, `TIME`, `TIMESTAMP`, `TIMESTAMPTZ`, `UUID`, `JSON`. Engine: widen PG `oid_to_tag` + value extraction and MySQL's column-metadata→tag map past the M0 set (each still keyed on **column metadata, never a guessed driver value**, per the S6 policy). Wire: pin the msgpack payload shape for each new tag and lock it with a **golden vector per tag** in both codecs (charter rule 2). PHP: the M1 `ValuePolicy` + the §9 value objects (`Ferro\Decimal` string-backed, `Ferro\Date`, `Ferro\Time`, `DateTimeImmutable` for TIMESTAMP/TIMESTAMPTZ, `Ferro\Uuid`, lazy `Ferro\Json`, `Ferro\U64`). Policy: implement the §9.1 pool-level knobs (`decimal`, `naive_datetime_zone`, `u64_overflow`, `uuid`) with the safe object defaults, and plumb the selection to the client — the `ValuePolicy` seam is also how the S8 DBAL tier gets **driver-native strings** instead of value objects (§9: "tiers apply their own ecosystem conventions on top").
- **Explicitly still deferred** (documented, §22): `ARRAY`, `INTERVAL`, `INET`, `VECTOR` — PG-exotic, not required for a green DBAL suite.
- **Gate:** a live round-trip of every newly-covered type on **both** PG and MySQL/MariaDB (write → read → exact-value assert, incl. DECIMAL precision and DATETIME fractional seconds — no silent truncation); a golden vector per new tag passing in the Rust **and** PHP conformance tests; every §9.1 policy exercised (incl. the `error` variants); the out-of-scope tags still raising a loud `Unsupported` naming the column. PHPStan L9; charter gates green.
...
### S10 — (conditional) bare-metal D12 re-run → `ext-php-rs` accelerator  *(depends: S5; gated by M1-D3)*
...
| R3 | The DBAL 4 upstream suite has cases Ferro deliberately won't support (native-PDO, dump creds, COPY) | The M1-D4 allow-list makes these DOCUMENTED breaks with rationale, not red tests; the incompat doc page is a first-class deliverable (S9). |
...
| R5 | `ext-php-rs` is a large optional subsystem built to chase a WSL2-inflated number | M1-D3: the bare-metal re-run settles the decision FIRST; the build (S10) is conditional and last; the accelerator is a transparent codec swap (no wire/contract change). |
