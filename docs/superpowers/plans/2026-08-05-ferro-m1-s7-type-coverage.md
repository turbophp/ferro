...
*Tags with zero implementation:* `ARRAY`(14), `INTERVAL`(15), `INET`(16), `VECTOR`(17) — PG-exotic, not required for a green DBAL suite. Also out of scope and still `Unsupported`: PG `TIMETZ`, PG enums/composites/ranges; MySQL/MariaDB `YEAR`/`BIT`/`ENUM`/`SET`/`GEOMETRY`/`VECTOR`; MariaDB's native `UUID` type (10.7+). **PG DOMAINs are NOT on this list** (T4b review F1 — the original "domains are deferred" claim was measured FALSE): PG resolves a domain to its BASE type in the `RowDescription` (`printtup.c` → `getBaseTypeAndTypmod`), so `Kind::Domain` never reaches the wire and a domain is supported iff its base is — a domain over `numeric(10,2)` reports OID 1700 → `DECIMAL`, one over `timetz` reports 1266 → `Unsupported`. Proven live in `pg_types_it.rs::domains_resolve_to_their_base_type_in_both_directions`.
...
| ~~**PG domains** — `Type::kind() → Kind::Domain(inner)` unwrap before the OID match~~ **NOT A CARRY — already correct in S7 (T4b review F1)** | ~~Every `information_schema` column is a **domain**~~ — true, but it costs nothing: PG resolves a domain to its **base type** when it builds the `RowDescription` (`printtup.c` → `getBaseTypeAndTypmod`), so the domain's own OID never reaches the wire and the pure-OID match already sees `sql_identifier`→`name`, `character_data`→`text`, … (`name`/`oid`/`"char"` are the *catalog scalar* carry in the row above — that is what actually gates introspection, not domains) | **0 h.** No unwrap, no recursion guard, no `Type` threading needed for this. A domain is supported iff its base OID is; measured live (`numeric(10,2)` domain → 1700 → `DECIMAL`; `timetz` domain → 1266 → `Unsupported`) and regression-locked by `pg_types_it.rs::domains_resolve_to_their_base_type_in_both_directions`. |
...
20. **Pure-OID matching misses enums/composites/ranges** (they arrive with a CUSTOM, database-local OID). They stay `Unsupported` in S7 — do not attempt `kind()` unwrapping in this slice; it gets a §22.2 line in Task 9. **(T4b review F1 correction: DOMAINs were wrongly grouped here.** PG resolves a domain to its base type in the `RowDescription` — `printtup.c` → `getBaseTypeAndTypmod` — so `Kind::Domain` never materialises at the wire and the pure-OID match already handles domains correctly in both directions. No unwrap exists to write; measured live and locked by `pg_types_it.rs::domains_resolve_to_their_base_type_in_both_directions`.)
...
Assert `Unsupported` — naming the column/type — for: PG `interval`, `inet`, `int4[]`, `timetz`, an enum, a domain over **`timetz`** (T4b review F1: a domain over *numeric* is **supported** — PG reports the base type — so asserting `Unsupported` for it would be wrong; the domain case that must still be refused is one over an unsupported base); MySQL/MariaDB `YEAR`, `BIT(8)`, `ENUM`, `SET`; MariaDB's native `UUID` type. And that the client raises a named `ProtocolException` for `TAG_ARRAY`/`INTERVAL`/`INET`/`VECTOR`.
...
- **§22.2** — one entry for the slice: the text-canonical wire decision and *why* (PurePacker cannot decode maps/ext; `str`/`bin` indistinguishable in PHP; precision/display-scale preservation); the implemented-set-in-the-hash skew decision; the MySQL UTC pin and its `NOW()` consequence; the naive/instant bind rule; the **documented engine asymmetries** — MySQL 8 has no native UUID type, **MariaDB `JSON` classifies as `TEXT`** (it is `LONGTEXT` + a `json_valid()` CHECK with no recoverable metadata) so S8's DBAL `json` mapping must be designed against it, MariaDB's native `UUID` stays `Unsupported`, and MySQL 8's default `sql_mode` blocks live zero-date inserts (unit-tested instead); the still-deferred tags (`ARRAY`/`INTERVAL`/`INET`/`VECTOR`, PG `TIMETZ`/enums/composites/ranges, MySQL `YEAR`/`BIT`/`ENUM`/`SET`) — **and that PG DOMAINs are NOT among them** (PG reports the base type in the `RowDescription`, so a domain is supported iff its base is; T4b review F1); the deliberately-thin (in fact **absent**) engine-side policy plumbing with its rationale; and the **three sized S8 carries** from the Global Constraints deferral table — PG catalog scalars (~1 h, `name`/`oid`/`"char"`, which is what actually gates `information_schema` introspection), the **narrowing bind path** (~4 h, the highest-frequency S8 blocker), and a `Ferro\Bytes` / binary-bind marker so `TAG_BYTES` is reachable from PHP at all (~2 h). *(Was four: the "PG domains via `Kind::Domain(inner)`, ~2 h" carry was measured FALSE and struck.)*
