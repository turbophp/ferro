...
**Interfaces produced:** `Responder::send_head(&self, cols) -> Result<(), WaitAborted>` + `Responder::send_data(&self, rows) -> Result<(), WaitAborted>` (non-consuming — the Responder keeps its one-shot terminal role); a new writer channel item that carries an `OutFrame` **plus** its `CapReserve` guard. **`HandlerFn` is UNCHANGED** (M5 — additive via `Responder`, not a new dispatch arg).

> **IMPLEMENTATION NOTE (supersedes the `OutFrame::Stream` shorthand used below):** `OutFrame` is a plain struct `{ header: Header, payload: Bytes }` (session/codec.rs) — it is NOT an enum and cannot gain a `Stream` variant, and it must stay pure for the codec/golden-vector tests. So realize the "`OutFrame::Stream { payload, _cap }`" shorthand as a **wrapper channel item**: `struct ControlMsg { frame: OutFrame, cap: Option<CapReserve> }`. The writer's `control_rx` becomes `Receiver<ControlMsg>`; it does `sink.send(msg.frame).await` then the `msg` (and its `cap`) drops → cap release. This ripples to EVERY `control_tx` sender (reader-loop control frames, diagnostics, the reserved terminal `OwnedPermit<ControlMsg>`, the supervisor's terminal send) — each non-stream send wraps its `OutFrame` as `ControlMsg { frame, cap: None }`. A streamed `send_data`/`send_head` may enqueue via either `control_tx.send(ControlMsg{frame, cap: Some(guard)}).await` (simplest — the guard-in-message makes it leak-safe if the send is cancelled) or a pre-reserved `reserve_owned` permit (matches the terminal path); both are correct because the release is tied to the `ControlMsg`'s drop.

- `writer.rs`: change the `control_rx` item to `ControlMsg { frame: OutFrame, cap: Option<CapReserve> }` (see the implementation note above). The writer writes `msg.frame` to the socket, then the `ControlMsg` (and its `cap`) drops → `CapReserve::drop` releases the cap (M6 release point is the message drop, not an explicit call). **Amend the doc comment** (lines ~3-7) that anticipated a "second, credit-limited data channel with control prioritized over data": the decision is the single ordered conduit (DATA + terminal share `control_tx` for FIFO/B4); the priority-split is DEFERRED (charter rule 5). (Also add a SPEC §22 deviation line — see the shared commit note.)
