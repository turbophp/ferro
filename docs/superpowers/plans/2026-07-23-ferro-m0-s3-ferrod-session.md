...
- a long-lived **writer task** fed by TWO channels: a small **control channel** (`mpsc`, capacity ~= max_inflight+8) for HELLO_ACK / PONG / **every terminal END** / session-fatal errors, and a **data channel** (bounded, credit-limited) for streamed result frames (produced starting S5). WINDOW_UPDATE and GOODBYE are inbound-only control frames — applied/acted on by the reader loop, never replied to (there is no WINDOW_UPDATE_ACK method). The writer drains control first, then data. **Terminals and control are never blocked by stream backpressure** — this is how Blocker-3 (silently-dropped mandatory terminal) is prevented.
...
**Terminal-delivery refinement (v2.1): the supervisor is the SOLE terminal-sender.** The handler never sends its own terminal frame — it *declares* its outcome via the consuming `Responder` (stores `Outcome::Ok(body)/Error(ep)/Cancelled` into a shared cell + sets `terminated`). During execution the handler may stream DATA frames via the data channel (S5), but the mandatory terminal END is sent exactly once BY THE SUPERVISOR (using a control permit reserved at insert) after the handler task joins: it sends the declared outcome, or synthesizes `Outcome::Error{distinct "no-terminal" code}` if the handler panicked or declared nothing. This makes exactly-one-END structurally unforgeable — one sender, one send, always delivered — with no double-send and no backpressure-drop.
...
- **As realized (T4):** the reader loop does NOT await handler handles inline (that would block multiplexing — PING-while-in-flight, T5). Instead, per request-bearing frame the reader reserves the control permit, then spawns a **detached per-request supervisor task** which spawns/owns the handler `JoinHandle`, `await`s it, and is the SOLE terminal-sender (v2.1 refinement). When the handle resolves: if the task panicked (`JoinError::is_panic()`) OR declared nothing, the supervisor synthesizes exactly one terminal `Outcome::Error` with a distinct `errc::PROTOCOL` "supervisor-synth" marker; else it sends the declared outcome. It then removes the registry entry. This does not rely on `Drop`-during-unwind. The writer task outlives all supervisors: each supervisor holds an `OwnedPermit` (a `Sender` clone), so `writer_handle.await` cannot complete while any supervisor is outstanding.
...
- **Exactly one END per request-bearing request** (SPEC §5.2, charter rule 4), scoped to services SQL/TX/STREAM. Core control/liveness responses (HELLO_ACK, PONG) are **non-terminal `flags=0` frames**; WINDOW_UPDATE and GOODBYE are inbound-only control frames that produce no ack at all (there is no WINDOW_UPDATE_ACK method). None of these enter the registry or are subject to one-END. Guaranteed by the supervisor (panic + no-terminal both covered); the diagnostic rejection of a reused id is a separate frame, not part of the original's lifecycle.
