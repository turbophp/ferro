...
- `classify_one_pg(stmt, pin_functions, pin_on_unknown) -> Option<PinTrigger>` (first match wins):
  ...
  5. leading `CREATE`: scan the leading modifier keywords — skip an optional `OR REPLACE` **and** an optional `GLOBAL`/`LOCAL` (PG grammar: `CREATE [OR REPLACE] [GLOBAL|LOCAL] {TEMP|TEMPORARY} [RECURSIVE] <object>`) — if `TEMP` or `TEMPORARY` then appears before the object kind → `Some(Temp)` (ANY temp object: `TABLE`/`VIEW`/`SEQUENCE`/`MATERIALIZED VIEW`/…). This MUST catch `CREATE OR REPLACE TEMP VIEW`/`CREATE OR REPLACE TEMPORARY VIEW` (verification C1 — a missed `OR REPLACE TEMP VIEW` leaks a session temp object). Plain `CREATE TABLE`/etc. (no TEMP) → fall through (safe).
