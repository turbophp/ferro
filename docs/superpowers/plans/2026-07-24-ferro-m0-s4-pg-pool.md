...
## Plan v2 — verification fixes (wf_7cb260d1, FIX_FIRST: 2 blockers + 5 majors)

These decisions SUPERSEDE any conflicting task text below; implement them:

- **B1 — no async in `Drop` (the async ROLLBACK cannot run in a synchronous `Checkout::Drop`).** Model: `Checkout::Drop` is FULLY SYNCHRONOUS — it returns the conn to the pool's idle set and sets per-conn flags (`tx_open`, `tainted`/`needs_reset`). The async cleanup happens at the START of the NEXT `checkout()` (async context): before handing out an idle conn, if it is flagged `tx_open` → run `ROLLBACK` (defensive), if `tainted`/pinned-served → run the reset (`DISCARD ALL`), and if the cleanup fails or the conn is dead → evict + reconnect. This "recycle-on-next-checkout" is the idiomatic hand-rolled solution. Rewrite `defensive_rollback_on_drop`: drop a Checkout with `tx_open` set, then the NEXT checkout of that conn performs the async ROLLBACK — assert the fake recorded `ROLLBACK` and the conn is `Unpinned`/clean (synchronize on the checkout, NOT on inline Drop).
- **B2 — `PoolBackend` MUST use `#[async_trait]`** (not bare async-fn-in-trait). The background reaper (Task 3) `tokio::spawn`s a future that awaits `PoolBackend::ping` over generic `B` — that future must be `Send`, which bare async-fn-in-trait does not guarantee. `#[async_trait]` boxes futures as `Send` by default. (Add `async-trait` to `ferro-pool`'s deps.) Not a "pick per toolchain" choice.
- **M1 — guard layering:** the raw `PoolBackend::simple_query` (trait method) is UNGUARDED and used ONLY by the pin hook (`begin_tx`/`commit_tx`/`rollback_tx`) and internal reset. Add a SEPARATE guarded public entry `Checkout::exec(sql)` (the user-facing path) that rejects bare tx-control. `bare_tx_control_..._rejected` calls `Checkout::exec`; `begin_tx` calls the raw trait method. (This resolves the contradiction where a guard on the single method would reject the pin machine's own BEGIN.)
- **M2 — tx-control guard covers Postgres synonyms:** match case-insensitively on the leading keyword(s) after skipping whitespace/comments: at minimum `BEGIN`, `START TRANSACTION`, `SAVEPOINT`, `COMMIT`, `END`, `ROLLBACK`, `ABORT`, `RELEASE`, `PREPARE TRANSACTION`. Add a `START TRANSACTION` case to the reject test. (Not a full lexer — `START TRANSACTION` is the load-bearing missing open-tx verb.)
- **M3 — reaper vs paused-clock tests:** give the pool a constructor/config that DISABLES the reaper (`reap_interval = None` / `Pool::without_reaper`) for the deterministic `start_paused` non-reaper tests, so auto-advance cannot fire the reaper and recycle a conn a test expects to survive. Run the live reaper only in reaper-specific tests (current-thread; add a `yield_now`/observed-await sync point after `advance` before asserting).
- **M4 — killed-backend test detects death via a ROUND TRIP, not `is_closed()`:** the checkout health check is cheap (`is_closed`/age, no round trip) and the driver sets `closed` asynchronously after EOF, so `is_closed` right after a kill races. Use the deterministic SECOND-connection `pg_terminate_backend(<pid>)` (not `pg_terminate_backend(pg_backend_pid())` which races its own result), then detect the dead conn on the NEXT USE (a `ping`/query that errors) → evict → surface `ConnectionLost` (Retryable), and assert the user statement was NOT re-run (charter rule 3).
- **M5 — connect-failure semantics:** a `connect()` failure on the checkout path surfaces `PoolError::ConnectionLost` (Retryable) to the caller IMMEDIATELY (no hidden retry loop in checkout — the client retries per policy). Jittered exponential backoff applies to the REAPER's background reconnection attempts, NOT to blocking a checkout. `connect_backoff_then_error` unit-tests the backoff SCHEDULE helper (assert the delays), separate from checkout. `checkout_timeout` bounds only the wait-for-a-permit/idle-conn, yielding `PoolError::Timeout`.
- **m1 — `pg_tx_pins_single_backend_pid` with `max_size >= 2` + a CONCURRENT checkout:** assert the tx keeps both statements on the pinned pid WHILE the concurrent checkout gets a DIFFERENT pid (proves pinning prevented load-balancing off the pinned conn) — the near-tautological same-guard/same-pid version cannot fail.
- **m2 — `queue_us` observability:** assert `queue_us > 0` on a successful-but-QUEUED checkout (a second checkout that waits for a permit then succeeds after the first releases), since the timed-out path returns `Err` with no `stats()` to read.
...
- **Defensive release (per v2/B1 — NO async in Drop):** `Checkout::Drop` is synchronous — it returns the conn to idle and, if the tx-open flag is set, marks it so the NEXT `checkout()` runs the async `ROLLBACK` (recycle-on-next-checkout) before reuse, or evicts it. Drop itself never awaits.
- **Guard (per v2/M1 + M2):** the raw `PoolBackend::simple_query` is unguarded (pin-hook/internal only); a separate guarded public entry `Checkout::exec(sql)` rejects bare tx-control — matching `BEGIN`/`START TRANSACTION`/`SAVEPOINT`/`COMMIT`/`END`/`ROLLBACK`/`ABORT`/`RELEASE`/`PREPARE TRANSACTION` (case-insensitive leading keyword) — as `PoolError::Unsupported`, so the stub cannot be bypassed (the real path is the TX service).
