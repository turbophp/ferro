...
| A5 | Re-run `testkit/dbal-suite.sh` on all three backends; record numbers | IN-FLIGHT (measurement running) | DISPATCHED 2026-09-09 12:29 UTC: `dbal-suite` workflow run [34351348533](https://github.com/turbophp/ferro/actions/runs/34351348533), family=all, on post-merge `main` (`6d507de` — carries A2+A3+B1a+B1b). Remaining work when it completes: pull both logs per family from the artifacts, record numbers + ordered failure sets into `docs/dbal-suite/` + this ledger + CLAUDE.md, diff against PG 296/374, MySQL 372/385, MariaDB 371/384; state plainly what still misses the §14 bar and why (SQLite has no backend until C3; the ORM suite is C-phase). A2 should move ~50 PG tests, A3 ~16 more; B1b closes the never-buffer clause. |
