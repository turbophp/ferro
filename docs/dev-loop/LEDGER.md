...
| # | Item | State | Notes |
|---|------|-------|-------|
| C1 | Eloquent tier (`ferro/laravel`) + PDO shim | SCOPED — see `docs/dev-loop/PHASE-C-SCOPE.md` | §15; Illuminate `Connection` execution layer only, stock Grammar/Processor. Sliced C1a–C1e on the B2 model, each premise marked verified/UNVERIFIED. **Most C1 carries are already paid** (`fetchRaw`/`fetch:none`, `streamRaw` on BOTH families since B2, the imperative tx trio, `lastInsertId`, `server_version`) — the S8a-style carry slice is much smaller than S8b's was. **One real gap found while scoping:** `selectResultSets()` has NO wire representation (`ExecOk` carries exactly one `cols`+`rows`), so multiple result sets are either a `/proto` change or a documented incompatibility — C1a decides which, first, because the answer changes C1b. |
