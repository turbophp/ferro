...
| B2b-2b | MySQL owned-conn `RowStream` + capability flip + ferrod | OPEN | Now a small slice on the B2b-2a husk: a real `MysqlRowStream` that owns the parked `Conn`, a `reclaim_stream` override that `unpark`s it via B2a's `into_conn`, flip `supports_row_streaming()` → true, repoint `mysql_it.rs`'s two refusal tests, add the live incremental proof. Remaining blocking criteria: (c) an ABANDONED owning stream (Drop-net path) must end up discarded — B2b-2a's `is_closed`-when-parked is what delivers this, so verify it live; and FB-3b (the still-unbounded drain inside `finish`) is now fixable here, where a live MySQL can prove the behavior. |
