...
- **FB-3 (HIGH, structural) — un-timed `handle.finish()` can strand the terminal END.**
  ...
  rather than creating it. **Fix belongs in B2b-2** (a bound whose failure path must interact
  correctly with a parked MySQL conn — untestable against the fake).
