...
pub struct InnerClient {
    ...
    /// FERRO M1-S1 fork (see `/UPSTREAM_PR.md`, drop when upstream merges): the same atomic the
    /// `PostgresCodec` (owned by the spawned `Connection` driver) writes on every `ReadyForQuery`
...
impl Client {
    ...
    /// FERRO M1-S1 fork (see `/UPSTREAM_PR.md`, drop when upstream merges): the raw `ReadyForQuery`
    /// transaction-status byte last reported by the server — `b'I'` (idle), `b'T'` (in a healthy
