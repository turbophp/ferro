...
impl Opts {
    ...
    pub(crate) fn get_capabilities(&self) -> CapabilityFlags {
        let mut out = CapabilityFlags::CLIENT_PROTOCOL_41
            ...
            // FERRO M1-S6 fork (see `/UPSTREAM_PR_MYSQL_ASYNC.md`): advertise CLIENT_SESSION_TRACK
            // unconditionally. The MySQL server only APPENDS session-state-info (system-variable,
            // schema, transaction-state, GTID trackers) to an OK packet when the client negotiated
            // this bit at handshake. Without it, `OkPacket::session_state_info()` is ALWAYS empty and
            // Ferro's MySQL pin engine — which reads those trackers as the authoritative signal for
            // protocol-invisible session mutations (the MySQL analog of Postgres's ReadyForQuery
            // status byte, M1-S1) — is dead at the handshake. This is the single load-bearing bit;
            // stock `mysql_async` omits it and exposes no `Opts`/`OptsBuilder` hook to add it.
